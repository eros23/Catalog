...
<div class="row-fluid">
  <div class="span12">
    ...
    while (list($lid, $date, $order_number, $ip, $uname, $order_description, $ship_info, $total_description, $partecipant_order, $comment_desc, $ship_date, $paid, $ordered, $order_status, $memo) = $db->sql_fetchrow($result)) {
      ...
      $link_user = '<a href="' . $admin_file . '.php?op=Users&chng_uid=' . $user_id . '">';
      $memo_edit = '<a href="' . $admin_file . '.php?op=catalog_memoedit&lid=' . $lid . '">'; ?>
           <tr>
			 ...
			 <?php if ($paid == 1) {
        echo '<a href="' . $admin_file . '.php?op=catalog_paidchg&amp;lid=' . $lid . '&amp;paid=1"><font color="#FF0000"><strong>' . _YES . '</strong></font></a>';
      } elseif ($paid == 0) {
        echo '<a href="' . $admin_file . '.php?op=catalog_paidchg&amp;lid=' . $lid . '&amp;paid=0"><strong>' . _NO . '</strong></a>';
      } ?>
             </span></td>
 	         <td align="center"><span class="atiny">
			 <?php if ($ordered == 1) {
        echo '<a href="' . $admin_file . '.php?op=catalog_orderchg&amp;lid=' . $lid . '&amp;ordered=1"><font color="#FF0000"><strong>' . _YES . '</strong></font></a>';
      } elseif ($ordered == 0) {
        echo '<a href="' . $admin_file . '.php?op=catalog_orderchg&amp;lid=' . $lid . '&amp;ordered=0"><strong>' . _NO . '</strong></a>';
      } ?>
             </span></td>
...
<div class="row-fluid">
  ...
  <div class="span4" id="user-list">
    ...
    <?php $resuser = $db->sql_query('SELECT uid,first_name,last_name,user_email,verified,DATE_FORMAT(registered, "%d/%m/%Y %H:%i:%s") as date_it 
		   					   FROM ' . $prefix . '_utenti
							   WHERE verified=1 
							   ORDER BY uid DESC 
							   LIMIT ' . $ofsbgn . ',' . $ofsppg . ''); ?>
	<ul class="list user_list">
	  ...
	  <li>
        <?php if ($verified == 1) {
        echo '<span class="label label-success pull-right sl_status">online</span>';
      } elseif ($verified == 0) {
